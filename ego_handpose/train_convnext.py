...
from models.PoolAttnHR_RTMCCHead_Pose_3D import PoolAttnHR_RTMCChead_Pose_3D
from models.ConvNeXt_Pose_3D import ConvNeXt_Pose_3D
from models.ViT_Pose_3D import ViT_Pose_3D
from models.ViTEVA02_Pose_3D import ViTEVA02_Pose_3D
...
def main(args):
    ...
    if cfg.MODEL.NAME in ["PoolAttnHR_Pose_3D", "PoolAttnHR_RTMCChead_Pose_3D"]:
    ...
    train_dataset = ego4dDataset(args, cfg, split=args.train_split, transform=transform)
    ...
    elif cfg.MODEL.NAME == 'ConvNeXt_Pose_3D':
        no_weight_decay_list = ['backbone.pos_embed', 'backbone.cls_token']
        param_groups = lrd.get_parameter_groups_for_convnext(
            model_without_ddp, cfg.TRAIN.WEIGHT_DECAY,
            skip_list=no_weight_decay_list,
            layer_decay=cfg.TRAIN.LAYER_DECAY,
            get_num_layer=lrd.get_num_layer_for_convnext, 
        )
        optimizer = torch.optim.AdamW(
            param_groups, lr=cfg.TRAIN.LR, weight_decay=0.05)
    else:
    ...
    for epoch in range(cfg.TRAIN.BEGIN_EPOCH, cfg.TRAIN.END_EPOCH):
        ...
        if (epoch % 10 == 0) or (epoch == cfg.TRAIN.END_EPOCH - 1) :
            # Save model weight
            torch.save(
                {
                    "epoch": epoch,
                ...
                os.path.join(
                    final_output_dir, f"epoch_{epoch+1}.pt"
                ),
            )
    
        logger.info(f"Best val loss: {best_val_loss:.5f}\t Best P-MPJPE: {best_val_p_mpjpe:.5f}")
