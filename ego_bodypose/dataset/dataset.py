...
class BodyPoseDataset(Dataset):
    def __init__(self, config, split="train", logger=None):
        ...
        self.logger.info(f"Building BodyPoseDataset {split} ...")
        ...
        self.depth_dir = os.path.join(self.config.DATASET.ROOT_DIR, self.config.DATASET.DEPTH_DIR) if self.use_depth else None

        # Hand path
        self.use_handpose = self.config.DATASET.get('USE_HAND_POSE', False)
        self.handpose_dir = os.path.join(self.config.DATASET.ROOT_DIR, self.config.DATASET.HAND_POSE_DIR) if self.use_handpose else None
        self.hand_max_dist = self.config.DATASET.get('HAND_MAX_DIST', 2.0)
        self.hand_clip_len = self.config.DATASET.get('NUM_FRAMES', 1)
    ...
    def _process_take_uid(self, take_uid):
        ...
        # hand pose dir
        handpose_dir = os.path.join(self.handpose_dir, take_name) if self.use_handpose else None
    ...
    def __getitem__(self, index):
        ...
        if data_dir["depth_dir"] is not None:
            ...
            for frame_id in frame_ids:
                ...
                img_path = f"{data_dir['depth_dir']}/{str(frame_id).zfill(5)}.jpg"
                if not os.path.exists(img_path):
                    print(f"{img_path} does not exist. skip this take. take_uid: {data_dir['take_uid']}")
                    return None

                img =  Image.open(img_path)
                depth_tensor = self.transform(img)
        ...
        # get the hand pose
        input_handpose = []
        if data_dir["handpose_dir"] is not None:
            frame_ids = win_frame_ids[-self.hand_clip_len:]

            file_path = f"{data_dir['handpose_dir']}/hamer_outputs.pkl"
            if not os.path.exists(file_path):
                print(f"{file_path} does not exist. handpose not exits: {data_dir['handpose_dir']}")
                return None
            
            with open(file_path, 'rb') as f:
                handpose_data = pickle.load(f)
            
            for frame_id in frame_ids:

                # get right hand
                right_hand = handpose_data['detections_right_wrt_cam'][str(frame_id).zfill(5)]
                right_hand_tensor = torch.full((21, 3), 100.0)
                if right_hand is not None:
                    right_hand = right_hand['keypoints_3d']
                    for i in range(len(right_hand)):
                        hand = right_hand[i]
                        if hand[0, 2] < right_hand_tensor[0, 2] and hand[0, 2] < self.hand_max_dist:
                            right_hand_tensor = torch.tensor(hand)
                if right_hand_tensor[0, 2] == 100.0:   # no valid hand
                    right_hand_tensor = torch.zeros((21, 3))

                # get left hand
                left_hand = handpose_data['detections_left_wrt_cam'][str(frame_id).zfill(5)]
                left_hand_tensor = torch.full((21, 3), 100.0)
                if left_hand is not None:
                    left_hand = left_hand['keypoints_3d']
                    for i in range(len(left_hand)):
                        hand = left_hand[i]
                        if hand[0, 2] < left_hand_tensor[0, 2] and hand[0, 2] < self.hand_max_dist:
                            left_hand_tensor = torch.tensor(hand)
                if left_hand_tensor[0, 2] == 100.0:   # no valid hand
                    left_hand_tensor = torch.zeros((21, 3))  

                # if right_hand_tensor[0, 2] < 2.0 and right_hand_tensor[0, 2] > 0.0001:  
                #     print("valid right hand", right_hand_tensor[0])
                # if left_hand_tensor[0, 2] < 2.0 and left_hand_tensor[0, 2] > 0:  
                #     print("valid left hand", left_hand_tensor[0])

                handpose_tensor = torch.stack((right_hand_tensor, left_hand_tensor), dim=0)  
                input_handpose.append(handpose_tensor)            
        else: 
            handpose_tensor = torch.zeros(2, 21, 3)
            input_handpose.append(handpose_tensor)

        input_handpose_tensors = torch.stack(input_handpose, dim=0)   # (20, 2, 21, 3)
        ...
        return {
            ...
            "inputs_handpose": input_handpose_tensors.numpy().astype(np.float32),
