...
class EgoVideoFusion(nn.Module):
    ...
    def forward(
        ...
        if self.use_image:
            ...
            # TODO: temporal fusion - avg, max, attention (transformer encoder)
            if self.temporal_fusion == 'mean':
                img_feat = torch.mean(x, dim=1)
            elif self.temporal_fusion == 'max':
                img_feat = torch.max(x, dim=1).values
            else:
        ...
        if self.use_depth:
            ...
            # TODO: temporal fusion - avg, max, attention (transformer encoder)
            if self.temporal_fusion == 'mean':
                depth_feat = torch.mean(x, dim=1)
            elif self.temporal_fusion == 'max':
                depth_feat = torch.max(x, dim=1).values
            else:
