...
class EgoVideoDepthFusion(nn.Module):
    def __init__(self, config):
        super().__init__()
        
        print('Buiding EgoVideoDepthFusion model ...')
        ...
        self.video_clip_len = config.DATASET.get("VIDEO_CLIP_LEN", 1)
        ...
        if self.use_image:
            ...
            if self.video_clip_len > 1:
                ...
                self.img_temporal_encoder = nn.Identity()

        # use depth image
        if self.use_depth:
            self.depth_backbone = nn.Identity()
            self.depth_feat_dim = config.MODEL.DEPTH_BACKBONE.EMBED_DIM
            if self.video_clip_len > 1:
                encoder_layer = nn.TransformerEncoderLayer(self.depth_feat_dim, nhead=nhead, batch_first=True, dropout=dropout_rate)
                self.depth_temporal_encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layer) 
            else:
                self.depth_temporal_encoder = nn.Identity()
    ...
    def forward(
        ...
        if self.use_image:
            ...
            for i in range(T):
                ...
                x = F.avg_pool1d(x.permute(0, 2, 1), kernel_size=49).squeeze(-1)
                img_feat_list.append(x)
            x = torch.stack(img_feat_list, dim=1)
            
            # TODO: temporal fusion - attention (transformer encoder)
            x = self.img_temporal_encoder(x)
            img_feat = x[:, -1, :]                
        else:
        ...
        if self.use_depth:
            depth_feat_list = []
            _, T, _, _= input_depth.shape
            for i in range(T):
                x = input_depth[:, i].squeeze()
                # x = self.depth_backbone(x)
                depth_feat_list.append(x)
            x = torch.stack(depth_feat_list, dim=1)

            # TODO: temporal fusion - , attention (transformer encoder)
            x = self.depth_temporal_encoder(x)
            depth_feat = x[:, -1, :]  
        else:
